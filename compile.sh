...
APPLICATION_VERSION=2.8.67
APPLICATION_VERSION_STRING=2.8.6
...
do_checks() {
    find "${DESTDIR}" -type f | while read file ; do
        if /usr/bin/file "${file}" | grep -q "Mach-O" ; then
            ...
            if nm -arch x86_64 -m "${file}"  | grep -v "_voucher" | grep -v "darwin_check_fd_set_overflow" | grep -v "_pthread_attr_set_qos_class_np" | grep -v "_pthread_mutexattr" | grep -v "from libc++" | grep -v "from libobjc" | grep -v "_dispatch_once_f" | grep -v "__availability_version_check" | grep -q "(undefined) weak external" ; then
...
do_removals() {
    ...
    sudo rm -rf ${DESTDIR}${PREFIX}/opt/buildX11*
}
...
do_meson_build src/xorg/app/appres EXEC
do_meson_build src/xorg/app/bitmap EXEC
do_meson_build src/xorg/app/editres EXEC
do_autotools_build src/xorg/app/fonttosfnt EXEC
do_meson_build src/xorg/app/fslsfonts EXEC
do_meson_build src/xorg/app/fstobdf EXEC
do_meson_build src/xorg/app/iceauth EXEC
do_meson_build src/xorg/app/ico EXEC
do_meson_build src/xorg/app/listres EXEC
...
do_meson_build src/xorg/app/showfont EXEC
do_meson_build src/xorg/app/smproxy EXEC
do_autotools_build src/xorg/app/twm EXEC
do_meson_build src/xorg/app/viewres EXEC
do_autotools_build src/xorg/app/xauth EXEC
do_autotools_build src/xorg/app/xbacklight EXEC
do_meson_build src/xorg/app/xcalc EXEC
...
do_meson_build src/xorg/app/xkill EXEC
